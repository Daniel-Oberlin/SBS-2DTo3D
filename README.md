...
| `SBS Method` | The algorithm used to generate the Side-by-Side 3D image from the depth map. <br> - mesh_warping: Warps the image based on a 3D mesh derived from the depth map. Generally provides good results. <br> - grid_sampling: Samples pixels from the original image based on a grid distorted by the depth map. Can be faster but might produce different visual artifacts. <br><br>For HD images (1280 x 780), set Depth Map Input Scale to 0.8 - 1.0 <br>For FHD images (1920 x 1080), set Depth Map Input Scale to 0.5 − 0.8 <br>For 2k images, set Depth Map Input Scale to 0.4 − 0.5 <br>For 4k images, set Depth Map Input Scale to 0.3 − 0.4 <br>For 6k images.. set Depth Map Input Scale to 0.2 − 0.3 <br>For 8k images, set Depth Map Input Scale to 0.15 − 0.2 <br> |
